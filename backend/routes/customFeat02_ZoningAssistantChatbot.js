...
// Natural language questions ("Can I convert to retail on this lot?"); AI references local code and provides instant answers with confidence levels
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / cf-zoning-assistant-chatbot',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_zoning_assistant_chatbot`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-zoning-assistant-chatbot feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Zoning assistant chatbot'.
Context: Natural language questions ("Can I convert to retail on this lot?"); AI references local code and provides instant answers with confidence levels
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-zoning-assistant-chatbot/run error:', err);
