...
// Gap Feature: Violations without \'/violation
// Violations without /violation-severity-score (risk-rank violations for enforcement priority)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / gap-violations-without-violation',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Violations without \'/violation'.\nProject: AIPermitZoningCompliance.\nContext: Violations without /violation-severity-score (risk-rank violations for enforcement priority)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['violations-without-violation', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'violations-without-violation', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
