...
// New permit triggers analysis of impact on surrounding properties (traffic, density, parking); auto-generates notice list; flags objection-prone neighbors
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / cf-neighborhood-impact-analysis',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_neighborhood_impact_analysis`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-neighborhood-impact-analysis feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Neighborhood impact analysis'.
Context: New permit triggers analysis of impact on surrounding properties (traffic, density, parking); auto-generates notice list; flags objection-prone neighbors
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-neighborhood-impact-analysis/run error:', err);
