...
// Applicant uploads plans; AI auto-checks against common violations (setbacks, parking, frontage); highlights issues before submission; reduces review rework
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / cf-ai-plan-pre-screening',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_plan_pre_screening`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-ai-plan-pre-screening feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI plan pre-screening'.
Context: Applicant uploads plans; AI auto-checks against common violations (setbacks, parking, frontage); highlights issues before submission; reduces review rework
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-plan-pre-screening/run error:', err);
