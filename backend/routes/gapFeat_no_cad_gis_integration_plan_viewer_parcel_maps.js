...
// Gap Feature: No CAD/GIS integration (plan viewer, parcel maps)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / gap-no-cad-gis-integration-plan-viewer-parcel-maps',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No CAD/GIS integration (plan viewer, parcel maps)'.\nProject: AIPermitZoningCompliance.\nContext: No CAD/GIS integration (plan viewer, parcel maps)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-cad-gis-integration-plan-viewer-parcel-maps', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-cad-gis-integration-plan-viewer-parcel-maps', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
