...
// Gap Feature: Code without \'/code
// Code without /code-interpretation-assistant (NLP clarification of ambiguous code sections)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / gap-code-without-code',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Code without \'/code'.\nProject: AIPermitZoningCompliance.\nContext: Code without /code-interpretation-assistant (NLP clarification of ambiguous code sections)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['code-without-code', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'code-without-code', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
