...
// Gap Feature: Plans without \'/plan
// Plans without /plan-review-ai (AI pre-screens for common violations before human review)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / gap-plans-without-plan',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Plans without \'/plan'.\nProject: AIPermitZoningCompliance.\nContext: Plans without /plan-review-ai (AI pre-screens for common violations before human review)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['plans-without-plan', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'plans-without-plan', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
