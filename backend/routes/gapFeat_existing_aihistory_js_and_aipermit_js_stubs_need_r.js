...
// Gap Feature: Existing \'aiHistory.js\' and \'aiPermit.js\' stubs need real implementation
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / gap-existing-aihistory-js-and-aipermit-js-stubs-need-r',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Existing \'aiHistory.js\' and \'aiPermit.js\' stubs need real implementation'.\nProject: AIPermitZoningCompliance.\nContext: Existing aiHistory.js and aiPermit.js stubs need real implementation\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['existing-aihistory-js-and-aipermit-js-stubs-need-r', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'existing-aihistory-js-and-aipermit-js-stubs-need-r', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['existing-aihistory-js-and-aipermit-js-stubs-need-r']
    );
