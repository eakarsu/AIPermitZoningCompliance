...
// Nuance severity (life-safety vs. administrative); AI recommends enforcement action (warning vs. citation vs. stop-work)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / cf-violation-escalation-scoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_violation_escalation_scoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-violation-escalation-scoring feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Violation escalation scoring'.
Context: Nuance severity (life-safety vs. administrative); AI recommends enforcement action (warning vs. citation vs. stop-work)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-violation-escalation-scoring/run error:', err);
