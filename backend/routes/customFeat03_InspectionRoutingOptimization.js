...
// Predict which permits will fail initial inspection; prioritize field inspections; minimize re-inspection cycles
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPermitZoningCompliance / cf-inspection-routing-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_inspection_routing_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-inspection-routing-optimization feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Inspection routing optimization'.
Context: Predict which permits will fail initial inspection; prioritize field inspections; minimize re-inspection cycles
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-inspection-routing-optimization/run error:', err);
