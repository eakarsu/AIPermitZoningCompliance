...
// Gap (Non-AI): No CAD/GIS integration (plan viewer, parcel maps)
// No CAD/GIS integration (plan viewer, parcel maps)
import React, { useState } from 'react';

export default function GapNoCadGisIntegrationPlanViewerParcelMapsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-cad-gis-integration-plan-viewer-parcel-maps/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No CAD/GIS integration (plan viewer, parcel maps)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No CAD/GIS integration (plan viewer, parcel maps)</p>
