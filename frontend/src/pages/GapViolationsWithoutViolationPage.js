...
export default function GapViolationsWithoutViolationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-violations-without-violation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Violations without '/violation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Violations without /violation-severity-score (risk-rank violations for enforcement priority)</p>
