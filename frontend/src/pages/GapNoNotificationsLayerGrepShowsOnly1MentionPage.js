...
// Gap (Non-AI): No notifications layer (grep shows only 1 mention)
// No notifications layer (grep shows only 1 mention)
import React, { useState } from 'react';

export default function GapNoNotificationsLayerGrepShowsOnly1MentionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-notifications-layer-grep-shows-only-1-mention/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No notifications layer (grep shows only 1 mention)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No notifications layer (grep shows only 1 mention)</p>
