...
// Gap (Non-AI): No fee calculation engine
// No fee calculation engine
import React, { useState } from 'react';

export default function GapNoFeeCalculationEnginePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-fee-calculation-engine/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No fee calculation engine</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No fee calculation engine</p>
