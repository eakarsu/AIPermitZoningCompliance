...
export default function GapPlansWithoutPlanPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-plans-without-plan/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Plans without '/plan</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Plans without /plan-review-ai (AI pre-screens for common violations before human review)</p>
