...
// Gap (AI): Inspections without '/inspection
// Inspections without /inspection-schedule-optimize (predict which permits need inspection first)
import React, { useState } from 'react';

export default function GapInspectionsWithoutInspectionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-inspections-without-inspection/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Inspections without '/inspection</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Inspections without /inspection-schedule-optimize (predict which permits need inspection first)</p>
