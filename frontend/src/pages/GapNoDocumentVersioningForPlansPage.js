...
// Gap (Non-AI): No document versioning for plans
// No document versioning for plans
import React, { useState } from 'react';

export default function GapNoDocumentVersioningForPlansPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-document-versioning-for-plans/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No document versioning for plans</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No document versioning for plans</p>
